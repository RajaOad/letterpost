...
import { ArrowLeft, Clock, Feather, Calendar, MapPin, Flame, X, AlertCircle } from 'lucide-react'
...
export default function LetterView({ letter, onBack }) {
  ...
  const canDelete = isSender // Only sender can delete
  ...
  const handleDelete = async () => {
    ...
    console.log('Attempting to delete letter:', letter.id)
    setIsDeleting(true)
    setDeleteError(null)
    ...
    try {
      const { data, error } = await supabase
        ...
        .delete()
        .eq('id', letter.id)
        .eq('sender_id', user.id) // Safety check: only delete if sender
        .select()
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-gradient-to-b from-amber-100 via-amber-50 to-amber-100 rounded-sm shadow-2xl border-4 border-amber-800/20 p-12 text-center relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h2 className="font-serif text-5xl text-amber-950 mb-6 font-bold">En Route</h2>
            <p className="font-body text-xl text-amber-800 mb-4">
            ...
            <div className="my-10 p-8 bg-gradient-to-b from-amber-200/50 to-amber-100/50 rounded-sm border-2 border-amber-800/20 inline-block shadow-inner">
              <div className="flex items-center justify-center gap-3 text-amber-900 mb-3">
                ...
                <span className="font-serif text-xl tracking-wider">EXPECTED ARRIVAL</span>
              </div>
            ...
            <div className="mt-10 pt-8 border-t-2 border-amber-800/10">
              <p className="font-script text-4xl text-amber-900 mb-3">"Patience is bitter, but its fruit is sweet"</p>
              <p className="font-body text-amber-700 italic">— Aristotle</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12">
      ...
      <AnimatePresence mode="wait">
        {!isOpen ? (
          // Closed Envelope - Premium Design
          <motion.div
            key="envelope"
          ...
          // Open Letter - Authentic Vintage Stationery
          <motion.div
            key="letter"
